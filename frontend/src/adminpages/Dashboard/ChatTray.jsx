...
//   const [allUsers, setAllUsers] = useState([]); // <-- keep in local state
//   const [position, setPosition] = useState({ top: 70, right: 170 });
//   const trayRef = useRef(null);
//   const [messages, setMessages] = useState([]);
...
//   // Close tray on outside click / scroll
//   useEffect(() => {
//     const handleClickOutside = event => {
//       if (
//         trayRef.current &&
...
//     return {
//       name: user?.name || 'Unknown User',
//       image: user?.profileImage ? `${constant.IMG_URL}/${user.profileImage}` : emptyuser,
//     };
//   };
...
//                 </p>
//                 <p className="message-text">{message.message}</p>
//                 <button onClick={() => handleViewReport(message.reportId)} className="view-report">
//                   View Report
//                 </button>
//               </div>
//             </div>
...
const ChatTray = ({ isOpen, onClose, anchorRef }) => {
  ...
  useEffect(() => {
    if (!allUsers || allUsers.length === 0) {
      console.log('Fetching users in ChatTray...'); // Debug
      fetchUsers(); // Now loads data
    } else {
      console.log('Users loaded in ChatTray:', allUsers.length); // Debug
    }
  }, [fetchUsers, allUsers]);
            ...
            return (
              <div key={message.id} className="message-item">
                ...
                <div className="message-content">
                  ...
                  <p className="message-context">
                    Mentioned you in {message.code} : {message.kpiName?.join(', ') || 'N/A'}
                  </p>
                  <p className="message-text">{message.message}</p>
                  <button
                    onClick={() => handleViewReport(message.reportId)}
