...
const AllTabs = () => {
  ...
  useEffect(() => {
    const fetchAll = async () => {
      try {
        ...
        const uniqueUsers = Array.from(new Map(userRes.data.data.map(u => [u.email, u])).values());
  ...
  const DataTable = ({ title, headers, rows }) => (
    <div className="card shadow-sm rounded-3 border-0 mb-3">
      ...
      >
        <h6 className="mb-0 fw-semibold text-dark" style={{ fontSize: '17px' }}>
          {title}
        </h6>
      </div>
    ...
    switch (section) {
        ...
        return (
          <DataTable
            ...
            rows={data.electrical.map((e, i) => [
              ...
              e.banks
                ?.map(
                  b => `Brand: ${b.brand}, KVA: ${b.kva}, Detuned: ${b.detuned}, Shunt: ${b.shunt}`
                )
                .join(' | '),
      ...
      case 'pdf':
        return (
          <DataTable
            title="PDF Files"
            headers={['S.No', 'Filename', 'Original Name', 'File Path', 'Upload Date', 'User ID']}
